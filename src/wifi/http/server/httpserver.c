...
// If the line was too long to fit into the buffer, returned length will be negative, but
// the next line will still get found correctly.
static char* recv_next_line_buffered(int socket,
                                     char* buffer,
                                     int buffer_size,
                                     int* buffer_used,
                                     int* offset,
...
static bool send_all(int socket, const char* buf, int size) {
    while (size > 0) {
#if MEM_SIZE < 16384
        /*	As of SDK 1.4.0, lwIP running out of memory to allocate a network buffer on
         *TCP send permanently stalls the entire netconn. As it doesn't free up the
         *resources taken by that netconn, the effect quickly snowballs, rendering the
         *entire network stack unusable:
         *
         *	1. tcp_pbuf_prealloc() called by tcp_write() returns a NULL.
         *	2. tcp_write() returns ERR_MEM
         *	3. lwip_netconn_do_write() receives ERR_MEM and assumes that it needs to wait
         *for the remote side to acknowledge the receipt. So it begins waiting on the
         *netconn semaphore: sys_arch_sem_wait(LWIP_API_MSG_SEM(msg), 0)
         *  4. As we did not send out any meaningful data, the acknowledgement (normally
         *done in tcp_receive()) never happens, and the lwip_send() never returns.
         *
         *  You can easily detect this condition by checking lwip_stats.tcp.memerr. If the
         *value is not 0, the IP stack has run out of memory at some point and might get
         *stuck as described before.
         *
         *  Increasing MEM_SIZE generally solves this issue, although it might return if
         *multiple threads attempt to send large amounts of data simultaneously.
         **/
...
static void parse_and_handle_http_request(http_connection ctx) {
    ...
    if (!header_buf || header_buf_size < 32) {
        http_server_send_reply(
            ctx, "400 Bad Request", "text/plain", "Malformed headers", -1);

        return;
    }

    for (;;) {
        char* line = recv_next_line_buffered(ctx->socket,
                                             header_buf,
                                             header_buf_size,
                                             &header_buf_used,
                                             &header_buf_pos,
        ...
        if (len > 0 && !strncasecmp(line, "Host: ", 6) &&
            (len - 6) < (sizeof(host) - 1)) {
...
static void http_server_thread(void* arg) {
    ...
    while (true) {
        ...
        if (conn_sock >= 0) {
            http_connection cctx =
                pvPortMalloc(sizeof(struct _http_connection) + sctx->buffer_size);
            if (cctx) {
                cctx->server = sctx;
                ...
                int retries = 0;
                while (xTaskCreate(do_handle_connection,
                                   "HTTP Connection",
                                   configMINIMAL_STACK_SIZE * 4,
                                   cctx,
                                   tskIDLE_PRIORITY + 2,
                                   &task) != pdTRUE &&
                       retries < TASK_CREATE_MAX_RETRIES) {
                    printf("xTaskCreate failed, retrying...\n");
                    vTaskDelay(pdMS_TO_TICKS(TASK_CREATE_RETRY_MS));
                    retries++;
                }

                if (retries == TASK_CREATE_MAX_RETRIES) {
                    vPortFree(cctx);
                    xSemaphoreGive(sctx->semaphore);
                    cctx = NULL;
                    printf("Could not create connection task after retries\n");
                }
            }
...
http_server_instance http_server_create(const char* main_host,
    ...
    http_server_instance ctx =
        (http_server_instance)pvPortMalloc(sizeof(struct _http_server_instance));
    if (!ctx) {
        closesocket(server_sock);
        return NULL;
    }
...
void http_server_add_zone(http_server_instance server,
                          http_zone* zone,
                          const char* prefix,
...
void http_server_send_reply(http_connection conn,
                            const char* code,
...
http_write_handle http_server_begin_write_reply(http_connection conn,
                                                const char* code,
                                                const char* contentType,
...
void http_server_write_reply(http_write_handle handle, const char* format, ...) {
    ...
    int written = vsnprintf(conn->buffer + conn->buffered_size,
                            conn->server->buffer_size - conn->buffered_size,
                            format,
                            args);
    va_end(args);
