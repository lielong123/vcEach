...
void emulator::handle_pid_request(std::string_view command) {
    ...
    bool extended_pids = false;
    size_t pos = 2;
    if (pos + 2 <= command.size() && is_valid_hex(command.substr(pos, 2))) {
        extended_pids = false;
    } else if (pos + 4 <= command.size() && is_valid_hex(command.substr(pos, 4))) {
        extended_pids = true;
    } else {
        Log::error << "ELM327: Invalid PID request format: " << command << "\n";
        out << "?\r\n>";
    ...
    while (pos < command.size()) {
        ...
        if (!extended_pids) {
            // Standard PID format (e.g., "01 0C")
            if (pos + 2 > command.size() || !is_valid_hex(command.substr(pos, 2))) {
                break; // No more valid PIDs
            }

            uint8_t pid = hex::parse(command.substr(pos, 2));
            pos += 2;

            frame.data[0] = 2; // 2 bytes of data
            frame.data[1] = mode;
            frame.data[2] = pid;
            current_request.pid = pid;
        } else {
            // Extended PID format (e.g., "09 0200")
            if (pos + 4 > command.size() || !is_valid_hex(command.substr(pos, 4))) {
                break; // No more valid PIDs
            }

            uint16_t pid = hex::parse(command.substr(pos, 4));
            pos += 4;

            // Create CAN frame for this PID
            frame.data[0] = 3; // 3 bytes of data
            frame.data[1] = mode;
            frame.data[2] = pid >> 8;
            frame.data[3] = pid & 0xFF;

            // For now, store the last PID as current (could be enhanced to track all)
            current_request.pid = pid;
        }
        ...
        if (res < 0) {
            ...
            // Otherwise continue with the successful ones
            break;
        }
        request_count++;
    }

    // Check if we found any valid PIDs to request
    if (request_count == 0) {
        Log::error << "ELM327: No valid PIDs found in request: " << command << "\n";
        out << "?\r\n>";
        is_waiting_for_response = false;
        return;
    }

    // Final character might be the expected frame count
    if (pos < command.size() && is_valid_hex(command.substr(pos, 1))) {
        expected_frames = hex::parse(command.substr(pos, 1));
    }
}
...
void emulator::update_timeout(uint64_t now,
    ...
    // This gives 7/8 old value, 1/8 new value
    avg_response_time = ((avg_response_time * 7) + response_time) >> 3;
    // Multiply by 4 (left shift by 2)
    uint64_t new_timeout = avg_response_time << 2;
...
void emulator::process_can_frame(const can2040_msg& frame) {
    ...
    if (!params.monitor_mode) {
        ...
        } else if (params.adaptive_timing && received_frames == 1) {
            const auto diff = now - current_request.request_start_time;
            update_timeout(now, diff, current_request.service, current_request.pid);
        }
    }
...
void emulator::emulator_task(void* params) {
    ...
    while (self->running) {
        if (self->is_waiting_for_response) {
            if (xSemaphoreTake(self->mtx, pdMS_TO_TICKS(1)) == pdTRUE) {
